  ...
  while ((c = mark[p])) {
    if (c.toLowerCase() == 'b') {
      ...
      if (val > 0xffff || val < 0) {
        ...
        valBinary = String.fromCharCode(Math.floor((val % 0x10000) / 0x100)) + String.fromCharCode(val % 0x100);
        if (littleEndian) {
...
const unpack = (mark, str) => {
  ...
  if (l != str.length) {
    throwError("'unpack' error. Mismatch between symbol and string length. " + l + ':' + str.length);
    return [null];
  }
  let littleEndian;
  if (mark[0] == '<') {
  ...
  constructor(exifBinary) {
    ...
    this.convertValue = (val) => {
      ...
      if (t == 1) {
        ...
        throwError('Exif might be wrong. Got incorrect value ' + 'type to decode. type:' + t);
        return null;
      }
...
const remove = (imageBinary) => {
  ...
  const newSegments = segments.filter(function (segment) {
    return !(segment.slice(0, 2) == '\xff\xe1' && segment.slice(4, 10) == 'Exif\x00\x00');
  });
...
const dump = (originalExifObj) => {
  ...
  if ('1st' in exifObj && 'thumbnail' in exifObj && exifObj['thumbnail'] != null) {
    existFirstIfd = true;
    exifObj['1st'][513] = 1;
    exifObj['1st'][514] = 1;
  ...
  const zerothIfdLength =
    zerothIfdSet[0].length + Number(existExifIfd) * 12 + Number(existGpsIfd) * 12 + 4 + zerothIfdSet[1].length;
  let exifIfdSet,
    exifIfdBytes = '',
    exifIfdLength = 0,
    gpsIfdSet,
  ...
  if (existFirstIfd) {
    const pointerValue = TIFF_HEADER_LENGTH + zerothIfdLength + exifIfdLength + gpsIfdLength + interopIfdLength;
    firstIfdPointer = pack('>L', [pointerValue]);
    const thumbnailPointer = pointerValue + firstIfdSet[0].length + 24 + 4 + firstIfdSet[1].length;
    const thumbnailPointerBinary = '\x02\x01\x00\x04\x00\x00\x00\x01' + pack('>L', [thumbnailPointer]);
    const thumbnailLengthBinary = '\x02\x02\x00\x04\x00\x00\x00\x01' + pack('>L', [thumbnail.length]);
    firstIfdBytes =
      firstIfdSet[0] + thumbnailPointerBinary + thumbnailLengthBinary + '\x00\x00\x00\x00' + firstIfdSet[1] + thumbnail;
  }
  const zerothIfdBinary = zerothIfdSet[0] + exifPointer + gpsPointer + firstIfdPointer + zerothIfdSet[1];
  if (existExifIfd) {
    exifIfdBytes = exifIfdSet[0] + interopPointer + exifIfdSet[1];
  }
  return header + zerothIfdBinary + exifIfdBytes + gpsIfdBytes + interopIfdBytes + firstIfdBytes;
};
